...
async function parseResume(resumeText) {
  ...
  try {
    // Correct usage of @google/generative-ai
    ...
    const prompt = `
      You are an expert ATS (Applicant Tracking System) parser. Analyze the following resume text and extract the information in JSON format matching the schema below:
...
function getMockParsedResume(text = '') {
  ...
  // Simple regex to extract name (first line or capital words)
  const lines = text.split('\n').map(l => l.trim()).filter(Boolean);
  const full_name = lines.length > 0 ? lines[0].replace(/[^a-zA-Z\s]/g, '') : 'Jane Doe';
